...
function TemplateThumb({ template, accent }: { template: TemplateId; accent: string }) {
  ...
  if (template === "marina") {
    return (
      <div className="flex h-full w-full overflow-hidden bg-white">
        <div className="w-[34%] shrink-0 space-y-1.5 p-2" style={{ backgroundColor: "#12263a" }}>
          <div className="mx-auto h-7 w-7 rounded-full border-2 bg-slate-500" style={{ borderColor: color }} />
          <div className="h-1 w-3/4 rounded bg-white/40" />
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-1 rounded bg-white/20" />
          ))}
          <div className="h-1 w-2/3 rounded" style={{ backgroundColor: color }} />
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-1 rounded" style={{ backgroundColor: color, opacity: 0.55, width: `${60 + i * 12}%` }} />
          ))}
        </div>
        <div className="flex-1 space-y-1.5 p-2">
          <div className="h-2.5 w-4/5 rounded bg-slate-800" />
          <div className="h-1 w-1/2 rounded bg-slate-300" />
          <div className="h-4 rounded-md border border-slate-200" />
          <div className="mt-1 h-1.5 w-1/3 rounded" style={{ backgroundColor: color }} />
          {[...Array(4)].map((_, i) => (
  ...
  if (template === "milano") {
    return (
      <div className="h-full w-full overflow-hidden p-2.5" style={{ backgroundColor: "#f7f0f3" }}>
        <div className="flex items-start gap-2 border-b border-slate-300 pb-2">
          <div className="flex-1 space-y-1">
            <div className="h-1.5 w-2/3 rounded bg-slate-400" />
            <div className="h-2.5 w-full rounded" style={{ backgroundColor: color }} />
            <div className="h-1 w-1/2 rounded bg-slate-300" />
          </div>
          <div className="h-9 w-7 shrink-0 border-2 bg-slate-300" style={{ borderColor: color }} />
        </div>
        <div className="mt-2 flex gap-2">
          <div className="flex-1 space-y-1">
            <div className="h-1.5 w-1/3 rounded" style={{ backgroundColor: color }} />
            {[...Array(7)].map((_, i) => (
              <div key={i} className="h-1 rounded bg-slate-300/70" style={{ width: `${60 + ((i * 13) % 35)}%` }} />
            ))}
          </div>
          <div className="w-[34%] space-y-1 border-s border-slate-300 ps-2">
