...
export function CoverLetterPreview({ doc, className }: { doc: CoverLetterDocument; className?: string }) {
  ...
  return (
    <div
      ...
      dir={rtl ? "rtl" : "ltr"}
      className={cn(
        "relative min-h-[297mm] w-full overflow-hidden bg-white p-[20mm] text-[11pt] leading-relaxed text-neutral-800 shadow-sm",
        className
      )}
      style={{ aspectRatio: "210 / 297", fontFamily: "Georgia, 'Times New Roman', serif" } as CSSProperties}
    >
      <div className="border-b border-neutral-300 pb-4">
        <p className="text-[15pt] font-semibold tracking-tight text-neutral-900">
      ...
      <div className="mt-10 whitespace-pre-line text-[10.5pt] text-neutral-700">
        {[doc.company, doc.recipient, doc.companyAddress].filter(Boolean).join("\n")}
      </div>

      <p className="mt-10 text-[9.5pt] text-neutral-500">{formatLetterDate(doc)}</p>

      <p className="mt-8 font-semibold text-neutral-900">
